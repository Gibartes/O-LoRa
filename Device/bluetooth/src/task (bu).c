...
static void *outputTask(void *param){
	...
	while(1){
		...
		if(err<=0){
   		logWrite(tcb->Log,tcb->log,"[*] [Output] packet drop : [%d]-LEN:[%llu].",err,len);
		    continue;}    // IGNORE
...
static void *inputTask(void *param){
	...
	while(1){
		...
		if(len>DATA_LENGTH || res<0){
    		logWrite(tcb->Log,tcb->log,"[*] [Input] packet drop : [%d]-LEN:[%llu].",res,len);
		    continue;}    // IGNORE
        err = hashCompare(&msg,&data,DATA_LENGTH);
        if(err==0){
		...
		if(err<=0 && errno!=EAGAIN){
		    setMask(tcb,tcb->sig,STATUS_EXIT);
		    logWrite(tcb->Log,tcb->log,"[*] [Input] exit : ENO:[%d]-EC:[%d]-LEN:[%llu].",errno,err,len);
		    goto exit;
		}
	}
