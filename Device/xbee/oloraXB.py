...
def SetNodeIdentifier(fullpacket, ExecutorPipe, localxbee, xnet, ev3):
    ...
    """
    try:
        SetValueStr = SetValue.decode("utf-8")
        localxbee.set_node_id(SetValueStr);localxbee.apply_changes();localxbee.write_changes()
        ...
        # header
        #fullpacket[28] |= 0b01000000 # Set OPT |= 0b01000000 to notify error has occured
        # don't send anything when error occured -> android will automatically timeout in 10 seconds
    else : # SUCCESS
        ...
        datasection = bytes(localxbee.get_64bit_addr())+localxbee.get_parameter("NI")
        headersection = fullpacket[0:56]
        ...
        headersection[36:38] = (datalength+8).to_bytes(2,'big')
        # Send CMD result
        ExecutorPipe.send(headersection+datasection) # Data Section : MAC + NI
        print("current local Nick name : ",localxbee.get_parameter("NI"))  
...
def StartDisocveryProcess(fullpacket,ExecutorPipe, localxbee, xnet, ev3):
    ...
    while (i < NumberOfdevices):
        ...
        print("device addr : ",bytearray(devices[i].get_64bit_addr()))
        NI = (devices[i].get_node_id()).encode('utf8');print("device nickname : ",NI)         
...
def SendSyncUnicast(fullpacket,ExecutorPipe, localxbee, xnet, ev3):
    ...
    # 4. Hash Result packet ( header + datasection )
    # Hash datasection
    hashdatasection = datasection + bytearray(952-len(datasection))
    hashvalue = hashlib.md5(hashdatasection).hexdigest()
    # fetch front & end 8 bytes seperately + swap byte by byte
    ...
    # final send value
    hashvalue_final = front + end
    header[PACKET_HEADER_CONFIG.MASK_DC:PACKET_HEADER_CONFIG.MASK_DATA] = (hashvalue_final)
        
    # 5. Send CMD_Result back 
    ExecutorPipe.send(header+datasection)
...
def SwitchRoom(fullpacket, ExecutorPipe, localxbee, xnet, ev3): 
    ...
    try:
        xnet.del_discovery_process_finished_callback(callback_discovery_finished)
        del xnet
        localxbee.reset()
        localxbee.set_parameter("HP", SetValue[0:1]) # get input as bytearray
        localxbee.set_parameter("ID", SetValue[1:3]);localxbee.apply_changes();localxbee.write_changes()
    except (TimeoutException, InvalidOperatingModeException, XBeeException, ATCommandException) as e: # ERROR
...
def executor(CmdQueue, MsgQueue, ExecutorPipe,  ev3, PidQueue, ReseterPid, WaitPipe): 
    ...
    while True:
        ...
        try:
            ...
            # Create Xnet Instance & Configurexnet
            ...
            while (True):
                # 1. Fetch
                fullpacket = CmdQueue.get() # max 1008 byte fullpacket 56+952 bytearray
                ...
                if CMD_NUM == b'\x13':
                    ...
                    # create NEW xnet instance
                    xnet = localxbee.get_network();xnet.clear()
                    xnet.set_discovery_options({DiscoveryOptions.DISCOVER_MYSELF, DiscoveryOptions.APPEND_DD})
                    xnet.set_discovery_timeout(4)
                    #xnet.add_discovery_process_finished_callback(callback_discovery_finished)

                # 3.2 CMD!=switch room ->e.g start_discovery 
                else :
                    routine_dic[bytes(CMD_NUM)](fullpacket, ExecutorPipe, localxbee, xnet, ev3)
                    print("after non switching cmd")
                #except:
